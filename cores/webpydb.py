...
def sqllist(lst): 
    ...
    if isinstance(lst, basestring): 
        ...
        # _sql = ', '.join(lst)
        # strinfo = re.compile(r",\s+(left|right)\s+join")
        # b = strinfo.sub(' ', _sql)
        return ', '.join(lst)
...
class DB: 
    ...
    def _db_execute_sql(self, cur, sql_query):
        """executes an sql query"""
        self.ctx.dbq_count += 1
    ...
    def execute_by_sql(self, sql):
        ...
        if db_cursor.description:
            names = [x[0] if x[0].find("count")<0 else "counts"  for x in db_cursor.description]
            def iterwrapper():
                row = db_cursor.fetchone()
                while row:
                    yield storage(dict(zip(names, row)))
...
def database(dburl=None, **params):
    ...
    if dburl:
        params.update(dburl2dict(dburl))
    dbn = params.pop('dbn')
